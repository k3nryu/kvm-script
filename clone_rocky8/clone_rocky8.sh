...
cp /root/clone_rocky8/rocky8.xml $conf/"$name".xml
#qemu-img create -f qcow2 -b /root/clone_centos8/centos8.qcow2 $img/"$name".qcow2 >/dev/null
cp /root/clone_rocky8/rocky8.qcow2 $img/"$name".qcow2 
...
echo 'Congratulations you have successfully created a Rocky8!'
virsh start $name
virsh list --all
